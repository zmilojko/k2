...
class RacesControllerTest < ActionController::TestCase
  ...
  test "should create race" do
    assert_difference('Race.count') do
      post :create, race: { format: @race.format, name: @race.name, status: @race.status, time: @race.time }
    end
  ...
  test "should update race" do
    patch :update, id: @race, race: { format: @race.format, name: @race.name, status: @race.status, time: @race.time }
    assert_redirected_to race_path(assigns(:race))
  end
